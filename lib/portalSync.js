...
async function syncToPortal(lead) {
  ...
  try {
    ...
    const isRetail = lead.jobCategory === 'retail' || lead.source === 'Retail';
    const body = {
      ...
      // Carry repCode so the portal's JN push routes the lead to the rep's
      // board (Holly default when unassigned) instead of skipping the push.
      repCode: String(lead.repCode || '').toUpperCase(),

...
async function _load() {
  // Load into a local first and only swap the cache on success. A failed query
  // must NEVER silently empty the cache — an empty cache locks every rep out of
  // the gate. (Guard kept even though the query is the Oregon-correct original.)
  try {
    ...
    console.error('[repCodes] _load failed — keeping last-good cache of ' + _cache.length + ' codes:', e.message);
  }
}
...
// Leadership = anyone who runs the company side (ceo/admin/operator), i.e. the
// L10 leaders. Add a leader in the portal roster with one of these roles and
// they get leadership access automatically.
function isLeader(code) {
  const rep = validateRepCode(code);
  return !!(rep && (rep.role === 'admin' || rep.role === 'ceo' || rep.role === 'operator' || rep.role === 'sales_manager'));
}
...
module.exports = { listRepCodes, listProducingRepCodes, validateRepCode, isAdmin, isLeader, refreshRepCodes, repCacheStatus };

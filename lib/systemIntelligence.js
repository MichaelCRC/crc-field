...
const { listProducingRepCodes } = require('./repCodes');
...
async function analyzeLeadConversion() {
  ...
  // Rule 1 (production scoreboard) -- lead conversion measures rep
  // performance, so we want sellers only, not admin / marketing users.
  const reps = listProducingRepCodes();
  const bySource = {};
  const byRep = {};
